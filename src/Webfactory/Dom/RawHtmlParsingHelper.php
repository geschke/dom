...
//var_dump($xml);
         ...
         "\n" .
             '<body><p>This is my legacy PHP project...</p>' . "\n" .
             '<div id="hurz">lalalala</div> </body> </head></html>';
        ...
        try {
            ...
            throw new ParsingException($e->getMessage(), new \DOMDocument(), $xml); // workaround until testing finished
        }


        $d = $this->createDOMDocument();
        //   var_dump(trim($document->asXML()));



        $d->loadXML(trim($document->asXML()));
...
//var_dump($obj);
        ...
        if ($obj instanceof \DOMNodeList || $obj instanceof \DOMNode) {
            ...
            /*   $newDocument = $this->wrapFragment('', $declaredNamespaces ? : $this->implicitNamespaces);
          ...
          //    echo "newDocument";
            //  var_dump($newDocument);
              ...
              try {
                  ...
                  $document = new \SimpleXMLElement($newDocument);
              }
              catch (\Exception $e) {

                  throw new ParsingException($e->getMessage(), new \DOMDocument(), $newDocument); // workaround until testing finished
              }
...
//var_dump($s);
    ...
    public function createXPath(\DOMDocument $document, $namespaceMappings = null) {
        ...
        // $document not longer declared as Document
